---
name: img2blender
description: Turn an object or character reference image into a quality-gated Blender scene, `.blend` deliverable, and deterministic review render using a staged reconstruction pipeline.
license: Apache-2.0
version: 1.4.1
---

# img2blender — Image to Blender

> **Backend status:** Blender is the default backend and **Blender MCP is the default execution
> transport**. The model designs the scene, compiles an operation plan, performs each mutation in
> the user's live local Blender through Blender MCP, then inspects screenshots before accepting a
> pass. Generated Blender Python is a non-MCP fallback, not the primary workflow.

Rebuild the object visible in a reference image as a **code-driven procedural Blender scene**,
...
6. **Locked build passes** — only touch the currently unlocked pass:
   ...
   `forge/stage3_build/prepare_blender_mcp_plan.py object-sculpt-spec.json --out build/mcp-plan.json --blend build/object.blend --render build/review.png`
7. **Operate Blender through MCP.** Discover the configured Blender MCP server/tools at runtime;
   do not assume provider-specific tool names. Execute `build/mcp-plan.json` in order. After every
   mutation, persist the MCP response/screenshot as operation evidence. Plan creation alone is not
   scene creation. Stop and report the exact error if Blender MCP is missing or disconnected.
8. Inspect the MCP-captured viewport and `build/review.png`; correct geometry/material/camera issues
   in live Blender before recording the pass. Never claim success without save + render + screenshot
   evidence from Blender MCP.
